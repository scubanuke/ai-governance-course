...
- **Error class** — the kind of cognitive error an assessment targets: *safety-critical*, *quality-of-service*, or *compliance*. Orthogonal to the criticality band — the band says how much is at stake, the error class says which kind of failure the assessment is hunting. The Tiered Assessment Framework assesses across all three.

- **Command Broker (CB-)** — the construct that enforces the Bright Line in live operation, mediating every AI command before it can reach the controlled system, and the **arbiter of design basis collisions**: where both the FC-DB and the AI-DB have a legitimate claim on the same decision, the Broker settles it at decision speed, since neither basis outranks the other. Made trustworthy by independence from the AI, determinism/verifiability, and minimality. Distinct from the CDB- communications design basis.
- **Managed interface** — the relationship between the two design bases. They are not a hierarchy: the AI-DB inherits the FC-DB's *consequences*, not its *authority*. The merge is discharged by making the seam itself the governed object — an assignable duty to detect conflict before acting, with a fallback when conflict cannot be removed. Modeled on the nuclear safety/security interface (10 CFR 73.58).
...
- **TAF (Tiered Assessment Framework)** — the procedure that proportions assessment depth to criticality, assessing the cognitive-layer errors of generative AI. Uses two orthogonal scales: criticality band (Highest/Mid/Lowest) and error class (safety-critical / quality-of-service / compliance).
