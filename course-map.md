...
**The companion unit — Merging the Two Design Bases:** two bases meeting at a seam is an architecture, not yet a method. What is inherited is consequence, not authority: at the seam the bases are co-equal, and neither settles a collision by outranking the other. The merge is therefore a *managed interface* — an assignable duty to detect conflict before acting, with the Command Broker as the arbiter when the collision surfaces at runtime. This unit also carries the operational payoff: the AI-DB is the source document from which contract requirements and the acceptance test plan are derived. Written out, not stubbed.
Closes with: *In Your Sector.*
