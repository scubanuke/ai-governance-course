...
This matters because the consequence anchor set at the class level propagates into everything downstream — the design basis, the banding, the assessment. Anchor the class on strategic significance and the LNG deliverability buffer looks peripheral; anchor it on deliverability consequence and the strategically sited but substitutable marine terminal looks peripheral. The two framings produce different classes, different core members, and different design bases. There is no obviously correct choice, which is precisely why it remains an open issue rather than a settled convention.
