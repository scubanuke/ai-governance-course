# Branch — Policy Track

## Learning Objectives

After this module, a student will be able to:

- Produce a consequence-and-governance analysis that reckons with both design bases.
- Analyze AI-enabled influence and commodity-market-manipulation cases.
- Apply the three deterrence forms — punishment, denial, resilience.
- Argue the standards gap for a facility or sector.


For the learner who wants to **produce** a consequence and governance analysis. The Democratic Safeguards material, deterrence theory, and the standards-gap argument. Ends by pointing into the capstone with a policy deliverable.

## Units, in order

1. `00-consequence-governance-analysis.md` — the analytic method
2. `01-democratic-safeguards-aib.md` — the Autonomous Influence Broker
3. `02-market-manipulation.md` — AI-enabled market manipulation
4. `03-deterrence-forms.md` — punishment, denial, resilience
5. `04-standards-gap-nipp-hole.md` — the missing mandate
