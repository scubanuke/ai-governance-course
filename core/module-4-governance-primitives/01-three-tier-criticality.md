...
This is the same logic that functional safety has used for decades. In that world, a function is assigned a Safety Integrity Level — a SIL — according to the severity of what happens if it fails, and the SIL then dictates how much assurance the function must be built and proven to. The criticality-band model is the SIL-equivalent for AI: the band is set by the consequence reachable across the Bright Line, and the band then dictates the obligations. Borrowing the structure keeps this course connected to established engineering practice rather than inventing a scale from nothing.
...
The exact boundaries between bands are drawn per domain, but the ordering principle never changes: **a higher band means greater reach to worse consequences, and greater reach obligates greater rigor.**

## How to place a system

Placing an AI in a band is a reachability question, and you already have the tools to answer it. Take the consequence root. Trace the AI-facing paths to it — the same paths the Bright Line is drawn across. Ask how severe the reachable consequence is and how direct the path: how many independent barriers stand between this AI's failure and the consequence, and how much does the AI's own action erode them? The severity of the consequence and the directness of the reach together fix the band. Placement is not a vibe or a vendor claim; it is derived from the design basis, and it can be shown and defended.

## Where this feeds

The band is not an end in itself — it is an instruction to the rest of the system. It tells the Command Broker in the next unit how hard the Bright Line has to be held for this system. And it is the seed of the assessment machinery the OT branch develops with the **Tiered Assessment Framework (TAF)**, where this criticality-band model becomes an operational procedure for classifying and treating systems — paired there with an orthogonal second axis, the **error class** (safety-critical, quality-of-service, compliance), which names *what kind* of cognitive error the assessment targets. Carry the band forward; it is the AI-DB's statement of how much is at stake.
...
- *2026-07-31 — Reconciled to the Consequence-Severity vocabulary: the AI-DB severity scale is the **criticality-band model** (Highest / Mid / Lowest), retiring the numeric "Tier 1/2/3" language; noted the orthogonal **error-class** axis carried by the TAF. Method unchanged.*
